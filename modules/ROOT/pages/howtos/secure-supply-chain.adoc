...
The https://sigstore.dev/[Sigstore] project provides tools and infrastructure for this. It’s for validating the integrity of the artifact supply chain.
...
In the following sections, you need a few tools to be installed. These are so users can sign and verify OCI artifacts signatures. The examples show the use of https://docs.sigstore.dev/cosign/installation/[`cosign`] and https://github.com/kubewarden/kwctl[`kwctl`] utilities for signing and inspecting policies.

Users may also want to use GitHub to sign their policies. In which case, they need to install https://docs.sigstore.dev/cosign/installation/#github-action[Github actions]

Keyless signing uses the default https://github.com/SigStore/fulcio[fulcio] and https://github.com/sigstore/rekor[rekor] instances provided by the Sigstore project. Check the Sigstore documentation for details on how to use your own infrastructure for this, if needed.

== Signing policies

Kubewarden recommends using Sigstore’s https://github.com/sigstore/cosign[cosign] utility to signing policies. This section shows a key-based method of signing policies. Users need to generate a private-public key-pair for this. The generated keys help to verify if the signed artifacts came from the expected user. To generate this key-pair use this `cosign generate-key-pair` command:
