...
Please refer to link:01-intro-wasi.md[Introduction to WASI] for an overview of the WASI execution mode.

[NOTE]
====
You mark the policy as `raw` by using the `policyType` field in the `metadata.yml` configuration. Please refer to the link:../metadata.md[metadata] specification for more information.
======= Validation
...
Start by scaffolding the policy by using the https://github.com/kubewarden/go-wasi-policy-template[go WASI policy template].
